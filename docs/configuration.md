...
Reference a preset from a docblock with `@preset name` instead of retyping the same `@queryParam`/`@header` on every endpoint. See [PHPDoc Annotations](phpdoc-annotations.md#presets).
